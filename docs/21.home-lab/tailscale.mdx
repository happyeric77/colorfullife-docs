...
If a LAN device becomes reachable over its Tailscale IP but not over its normal local IP, check this case study:

- [Tailscale Subnet Router and Asymmetric Routing on a Home LAN](/archive/home-lab/tailscale-subnet-router-asymmetric-routing)
